...
import mfv.booking.repository.SeatRepository
...
@Service
class BookingService(private val bookingRepository: BookingRepository, private val seatRepository: SeatRepository) {
    ...
    fun bookSeat(seatId: Long, userId: Long, startTime: LocalDateTime, endTime: LocalDateTime): Booking? {
        if (isSeatAvailable(seatId, startTime, endTime)) {
            ...
            val booking = Booking(seatId = seatId, userId = userId, startTime = startTime, endTime = endTime)
